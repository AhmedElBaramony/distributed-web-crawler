        ...
        for msg in receive_messages_sqs(RESULT_QUEUE_URL, max_messages=5):
            ...
            if task_id in assigned_tasks:
                if url not in crawled_urls_set:
                    ...
                    logger.warning(f"[Result] Duplicate result received for already crawled URL: {url}")
                assigned_tasks.pop(task_id)
            else:
                logger.warning(f"[Result] Received orphaned result for task_id {task_id} → Ignored URL '{url}'")
                continue  # Skip processing child URLs from this result
        ...
        for task_id, (url, depth, assigned_time) in list(assigned_tasks.items()):
            if (now - assigned_time).total_seconds() > TASK_TIMEOUT:
                if url not in crawled_urls_set:
                    ...
                    logger.info(f"[Timeout] Skipped requeue — already crawled: {url}")
                assigned_tasks.pop(task_id)
